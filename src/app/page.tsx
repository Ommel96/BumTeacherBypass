      ...
      {showUpload && (
        <div className="mt-8 bg-white border border-[var(--border)] rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-serif text-xl font-bold">Upload Document</h2>
          ...
          {uploadResult ? (
            <div className="text-center py-6">
              ...
              <p className="font-semibold mb-1">{uploadResult.filename} uploaded!</p>
              <p className="text-sm text-[var(--text-muted)] mb-4">AI is processing your document. This may take a few minutes.</p>
              <div className="flex gap-3 justify-center">
                <Link href={`/documents/${uploadResult.id}`} className="bg-[var(--accent)] text-white px-4 py-2 rounded-lg no-underline font-medium hover:bg-[var(--accent-dark)]">View Document</Link>
                <button onClick={closeUpload} className="bg-white border border-[var(--border)] text-[var(--text)] px-4 py-2 rounded-lg font-medium cursor-pointer hover:bg-gray-50">Close</button>
              </div>
            </div>
          ) : (
            <form onSubmit={handleUpload}>
              ...
              >
                ...
                {file ? (
                  <div>
                    ...
                    {classifying && <p className="text-sm text-[var(--accent)] mt-1">Detecting module & topic...</p>}
                  </div>
                ) : (
                  <div>
                    ...
                    <p className="text-[var(--text-muted)]">Click to select a PDF or Word file</p>
                  </div>
                )}
              </div>
              ...
              <div className="mb-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-sm text-[var(--text)]">Auto-detect category</span>
                  {classifying && <span className="text-xs text-[var(--accent)] ml-2">Detecting...</span>}
                </div>
                {!autoDetect && (
                  <p className="text-xs text-[var(--text-muted)] mb-3">Fill in the category manually, or turn auto-detect on.</p>
                )}
              ...
              <button type="submit" disabled={uploading || !file} className="w-full flex items-center justify-center gap-2 bg-[var(--accent)] text-white px-5 py-3 rounded-lg font-semibold hover:bg-[var(--accent-dark)] transition-colors disabled:opacity-50 border-none cursor-pointer">
                {uploading ? <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"/>Uploading...</> : 'Upload & Process'}
              </button>
            </form>
          )}
        </div>
